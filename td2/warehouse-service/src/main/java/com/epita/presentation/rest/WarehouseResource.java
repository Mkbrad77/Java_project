...
public class WarehouseResource {
    ...
    @Path("/products")
    public Response getAllProducts() {
        return Response.ok(warehouseService.getAllProducts()).status(200).build();
    }
    ...
    @Path("/purchase")
    public Response purchaseProducts(List<ProductEntity> products) {
        if (products == null || products.isEmpty() || products.stream().anyMatch(p ->  p.getQuantity() <= 0)) {
            return Response.status(Response.Status.BAD_REQUEST).build();
        }
        String success = warehouseService.purchaseProducts(products);

        if (success.equalsIgnoreCase("no product matches name")) {
            return Response.status(Response.Status.NOT_FOUND).entity(success).build();
        } else if (success.equalsIgnoreCase("Not enough stocks")) {
